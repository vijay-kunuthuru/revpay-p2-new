...
public class WalletController {
        ...
        public record TransactionDTO(Long transactionId, Long senderId, Long receiverId, BigDecimal amount, String type,
                        String status, String description, LocalDateTime timestamp, String transactionRef) {
        }

        public record PaymentMethodDTO(Long id, String partialCardNumber, String expiryDate, String billingAddress,
                        boolean isDefault, String cardType) {
        }

        public record SimpleAmountRequest(@NotNull @Positive(message = "Amount must be positive") BigDecimal amount,
                        String description) {
        }

        public record AddFundsRequest(@NotNull @Positive(message = "Amount must be positive") BigDecimal amount,
                        @NotNull(message = "A valid Card ID is required to add funds") Long cardId,
                        String description) {
        }
        ...
        private PaymentMethodDTO mapCard(PaymentMethod c) {
                String masked = (c.getCardNumber() != null && c.getCardNumber().length() >= 4)
                                ...
                                : "INVALID CARD";
                return new PaymentMethodDTO(c.getId(), masked, c.getExpiryDate(), c.getBillingAddress(), c.isDefault(),
        ...
        public ResponseEntity<ApiResponse<BigDecimal>> getBalance(Authentication auth) {
                return ResponseEntity.ok(
                                ApiResponse.success(walletService.getBalance(getAuthenticatedUser(auth).getUserId()),
                                                "Balance retrieved"));
        }

        @PostMapping("/send")
        @Operation(summary = "P2P Money Transfer", parameters = {
        ...
        public ResponseEntity<ApiResponse<TransactionDTO>> addFunds(@Valid @RequestBody AddFundsRequest req,
                        Authentication auth) {
                Transaction t = walletService.addFunds(getAuthenticatedUser(auth).getUserId(), req.amount(),
                                req.cardId(),
                                req.description());
        ...
        public ResponseEntity<ApiResponse<TransactionDTO>> withdraw(@Valid @RequestBody SimpleAmountRequest req,
                        Authentication auth) {
        ...
        public ResponseEntity<ApiResponse<TransactionDTO>> payInvoice(@Valid @RequestBody InvoicePaymentRequest req,
                        Authentication auth) {
                Transaction t = walletService.payInvoice(getAuthenticatedUser(auth).getUserId(), req.getInvoiceId(),
                                req.getTransactionPin());
                return ResponseEntity.ok(ApiResponse.success(mapTx(t), "Invoice paid"));
        }

        @PostMapping("/request")
        public ResponseEntity<ApiResponse<TransactionDTO>> requestMoney(@Valid @RequestBody MoneyRequestPayload req,
                        Authentication auth) {
                Transaction t = walletService.requestMoney(getAuthenticatedUser(auth).getUserId(), req.targetEmail(),
                                req.amount());
                return ResponseEntity.ok(ApiResponse.success(mapTx(t), "Request initiated"));
        }

        @PostMapping("/request/accept/{txnId}")
        public ResponseEntity<ApiResponse<TransactionDTO>> acceptRequest(@PathVariable Long txnId,
                        @Valid @RequestBody PinPayload body) {
        ...
        public ResponseEntity<ApiResponse<TransactionDTO>> declineRequest(@PathVariable Long txnId,
                        Authentication auth) {
        ...
        public ResponseEntity<ApiResponse<TransactionDTO>> cancelOutgoingRequest(@PathVariable Long txnId,
                        Authentication auth) {
        ...
        public ResponseEntity<ApiResponse<Page<TransactionDTO>>> getIncoming(@PageableDefault(size = 10) Pageable p,
                        Authentication auth) {
                return ResponseEntity.ok(ApiResponse.success(
                                walletService.getIncomingRequestsPaged(getAuthenticatedUser(auth).getUserId(), p)
                                                .map(this::mapTx),
                                "Incoming requests fetched"));
        }

        @GetMapping("/requests/outgoing")
        public ResponseEntity<ApiResponse<Page<TransactionDTO>>> getOutgoing(@PageableDefault(size = 10) Pageable p,
                        Authentication auth) {
                return ResponseEntity.ok(ApiResponse.success(
                                walletService.getOutgoingRequestsPaged(getAuthenticatedUser(auth).getUserId(), p)
                                                .map(this::mapTx),
                                "Outgoing requests fetched"));
        }

        // --- 3. SEARCH & ADVANCED FILTERING ---

        @GetMapping("/transactions")
        public ResponseEntity<ApiResponse<Page<TransactionDTO>>> getHistory(@RequestParam(required = false) String type,
                        @PageableDefault(size = 20) Pageable p, Authentication auth) {
                User user = getAuthenticatedUser(auth);
                Page<Transaction> res = (type != null && !type.isEmpty())
                                ? walletService.getMyHistoryPaged(user, type, p)
                                : walletService.getTransactionHistoryPaged(user.getUserId(), p);
        ...
        public ResponseEntity<ApiResponse<Page<TransactionDTO>>> filter(
                        ...
                        @PageableDefault(size = 20) Pageable p, Authentication auth) {
                Page<Transaction> filtered = walletService.filterTransactionsPaged(
                                getAuthenticatedUser(auth).getUserId(), type,
                                startDate, endDate, status, minAmount, maxAmount, p);
                return ResponseEntity.ok(ApiResponse.success(filtered.map(this::mapTx), "Filtered results retrieved"));
        }

        @PostMapping("/transactions/filter")
        public ResponseEntity<ApiResponse<Page<TransactionDTO>>> filterUsingDTO(
                        @RequestBody TransactionFilterDTO filter,
                        @PageableDefault(size = 20) Pageable p, Authentication auth) {
                Page<Transaction> filtered = walletService.filterTransactionsPaged(
                                getAuthenticatedUser(auth).getUserId(),
                                filter.getType(), filter.getStartDate(), filter.getEndDate(), filter.getStatus(),
                                filter.getMinAmount(),
                                filter.getMaxAmount(), p);
                return ResponseEntity.ok(ApiResponse.success(filtered.map(this::mapTx), "Filtered results retrieved"));
        }

        @GetMapping("/transactions/search")
        public ResponseEntity<ApiResponse<Page<TransactionDTO>>> search(@RequestParam String keyword,
                        @PageableDefault(size = 20) Pageable p, Authentication auth) {
                return ResponseEntity.ok(ApiResponse.success(walletService
                                .searchTransactionsPaged(getAuthenticatedUser(auth).getUserId(), keyword, p)
                                .map(this::mapTx),
        ...
        public ResponseEntity<ApiResponse<WalletAnalyticsDTO>> getAnalytics(Authentication auth) {
                return ResponseEntity
                                .ok(ApiResponse.success(walletService.getSpendingAnalytics(getAuthenticatedUser(auth)),
        ...
        @GetMapping("/cards")
        public ResponseEntity<ApiResponse<Page<PaymentMethodDTO>>> getCards(@PageableDefault(size = 10) Pageable p,
                        Authentication auth) {
                return ResponseEntity.ok(ApiResponse.success(
                                walletService.getCardsPaged(getAuthenticatedUser(auth).getUserId(), p)
                                                .map(this::mapCard),
                                "Saved cards retrieved"));
        }

        @PostMapping("/cards")
        public ResponseEntity<ApiResponse<PaymentMethodDTO>> addCard(@Valid @RequestBody CardPayload payload,
                        Authentication auth) {
        ...
        public ResponseEntity<ApiResponse<PaymentMethodDTO>> updateCard(@PathVariable Long cardId,
                        @Valid @RequestBody CardPayload payload, Authentication auth) {
                PaymentMethod card = new PaymentMethod();
                card.setCardNumber(payload.cardNumber());
                card.setExpiryDate(payload.expiryDate());
                card.setBillingAddress(payload.billingAddress());

                PaymentMethod updatedCard = walletService.updateCard(getAuthenticatedUser(auth).getUserId(), cardId,
                                card);
